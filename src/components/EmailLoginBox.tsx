...
export default function EmailLoginBox() {
  ...
  const [me, setMe] = useState<{ id: string; email: string | null; role: Membership } | null>(null);
  ...
  const fetchMe = useCallback(async () => {
    ...
    try {
      ...
      const u = j?.ok && j?.authed ? j.user : null;
      ...
      setNeedsWalletLink(m === "DIAMOND" && !!u.email && !linked);
    } finally {
      setMeLoaded(true);
    }
  }, []);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Confirm session/cookie worked and refresh UI state
      await fetchMe();
    ...
    <div className="relative">
      ...
      {showLoggedIn ? (
        <div className="space-y-3">
          {needsWalletLink && (
            <div className="rounded-2xl border border-pink-500/40 bg-black/60 px-4 py-4">
